...
#pragma multi_compile _RECEIVE_SHADOWS _ON
#endif
...
void MainLight_half(float3 WorldPos, out float3 Direction, out float3 Color, out float ShadowAtten, out float DistanceAtten)
{
#if defined(SHADERGRAPH_PREVIEW)
    Direction = float3(0.5, 0.5, 0);
    Color = 1;
...
void AdditionalLights_half(float Smoothness, float3 WorldPosition, float3 WorldNormal, float3 WorldView, out float3 Diffuse, out float3 Specular)
{
    float3 diffuseColor = 0;
    float3 specularColor = 0;
    float4 White = 1;
